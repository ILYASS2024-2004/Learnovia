...
exports.getAll = (role) => async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute(`SELECT id, nom, email, date_naissance, adress FROM ${TABLES[role]}`);
    res.json(rows);
  } catch (err) {
...
exports.getOne = (role) => async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute(`SELECT id, nom, email, date_naissance, adress FROM ${TABLES[role]} WHERE id = ?`, [req.params.id]);

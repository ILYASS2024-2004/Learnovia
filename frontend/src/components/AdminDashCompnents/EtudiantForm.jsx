...
import { X, User, Mail, Calendar, MapPin, Lock, User2 } from 'lucide-react';

const EtudiantForm = ({ etudiant, onClose }) => {
 ...
 const {theme}=useThemeStore()
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      ...
      await updateUser('etudiant', etudiant.id, dataToSend);
    } else {
      await addUser('etudiant', dataToSend);
    }
    onClose();
  };
    ...
    <div className={`fixed inset-0 ${theme=="light"?" bg-black/20":"bg-white/20"} bg-black/20 bg-opacity-40 z-50 flex items-center justify-center`}>
      <div className="bg-base-100 p-6 rounded-xl shadow-xl w-full max-w-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="form-control w-full">
            <div className="label"><span className="label-text">Date d'inscription</span></div>
            <div className="relative">
              <Calendar className="absolute left-3 top-2 z-1 text-gray-400" />
              <input
                type="date"
                name="date_inscription_ecole"
                value={formData.date_inscription_ecole?.split('T')[0]}
                onChange={handleChange}
                className="input input-bordered w-full pl-12"
                required
              />
